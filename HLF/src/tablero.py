...
class Tablero:
    ...
    def __clear_(self):
        ...
        '''
        self.tablero = np.full(shape=(self.BOARD_SIZE, self.BOARD_SIZE), 
    ...
    def initialize_boats(self):
        ...
        for boat_size in self.BOAT_SIZES:
            while True:
                ...
                cond_pos = self.valid_position(coord_x, coord_y, orient, 
                                               boat_size)
                if cond_pos:
                    self.tablero = self.place_boat(coord_x, coord_y, orient, 
                                                   boat_size)
                    break
    ...
    def valid_position(self, coord_x, coord_y, orient, boat_size):
        ...
        if orient == "N":
            ...
            cond_over = np.any(tab[coord_x:final_row, coord_y] == self.BOAT_SIGN)
        elif orient == "E":
            final_col = coord_y + boat_size
            cond_border = (final_col <= self.BOARD_SIZE)
            cond_over = np.any(tab[coord_x, coord_y:final_col] == self.BOAT_SIGN)
        elif orient == "W":
